...
GLDevice::RendererInfo GLDevice::GetRendererInfo() {
    if (!GLDevice::m_RendererInfo.init) {
        GLDevice::InitRendererInfo();
    }

    return GLDevice::m_RendererInfo;
}
...
void GLDevice::SetOption(GLDeviceOption option, bool enable) {
    switch (option) {
        case eUseMTGL: {
            break;
        }
        ...
        case eShaderConstantBindings: {
            GLDevice::m_ShaderConstantBindings = enable;
            break;
        }
...
void GLDevice::StaticInit() {
    ...
    *static_cast<char *>(texture0->Map(0, nullptr, v30, GL_WRITE_ONLY)) = 0xFF;
    ...
    // GLDevice::m_DeviceTextures[1] = texture1;
...
void GLDevice::ApplyGLStates(const GLStates &states, bool force) {
    ...
    int32_t maxClipPlaneIndex = GLDevice::GetRendererInfo().unk36;
    ...
    int32_t maxColorMaskIndex = GLDevice::m_ExtColorMaskIndexed ? GLDevice::GetRendererInfo().max_color_attachments : 1;
...
void GLDevice::BlitFramebuffer(GLMipmap *src, const GLRect *srcRect, GLMipmap *dst, const GLRect *dstRect, GLEnum mask,
    ...
    this->SetShader(GLShader::eVertexShader, GLDevice::m_DeviceShaders[0]);
    this->SetShader(GLShader::ePixelShader, GLDevice::m_DeviceShaders[2]);
...
void GLDevice::Draw(GLEnum primitive, uint32_t a3, uint32_t a4) {
    // TODO
}
...
// 使用 QOpenGLContext 获取函数指针
    ...
    this->m_UseWindowSystemBuffer = a4 & (0x2 | 0x4);
    this->m_FlippedSystemBuffer = a4 & 0x4;
    this->m_ShaderCompiler = a4 & 0x8;
    this->m_WorkerDevice = a4 & 0x10;

    this->m_DebugName.assign(a3, strlen(a3));

    GLDevice::Set(this);

    GLTextureFormat v6 = GLTF_INVALID;
    ...
    if (this->m_ID == -1) {
        this->m_ID = GLDevice::m_Devices.size();
        GLDevice::m_Devices.insert(GLDevice::m_Devices.end(), this);
    } else {
        GLDevice::m_Devices[this->m_ID] = this;
    }
...
void GLDevice::LoadDefaultStates() {
    // Depth
...
void GLDevice::SetCullMode(GLEnum cullMode) {
...
void GLDevice::SetDepthBias(float constantBias, float slopeScaledBias) {
    // TODO
}
...
GLDevice::SetShaderConstants(GLShader::ShaderType shaderType, uint32_t index, const float *constants, uint32_t count) {
    ...
    if (GLDevice::m_ShaderConstantBindings) {
        if (!shader) {
            return;
        }
...
void GLDevice::SetUnpackClientStorage(bool enable) {
    // TODO
...
void GLDevice::UpdateFFPTexturing() {
    // TODO
}
