...
void GLVertexArray::ApplyGLStates(GLStates::VertexArrayObject &vao) {
    ...
    if (vao.position.enable) {
        ...
        glVertexPointer(vao.position.size, vao.position.type, vao.position.stride,
                                              vao.position.offset);
    ...
    for (int32_t i = 0; i < 8; i++) {
        ...
        if (vao.texCoord[i].enable) {
            ...
            glTexCoordPointer(vao.texCoord[i].size, vao.texCoord[i].type, vao.texCoord[i].stride,
...
void GLVertexArray::ApplyVertexFormat(GLDevice *device) {
            ...
            if (useVertexShader) {
                ...
                switch (attrib.slot) {
                    ...
                    case 13: {
                        ...
                        glActiveTexture(GL_TEXTURE0 + texCoordIndex);
                        glTexCoordPointer(
                                k_VertexTypeInfo[attrib.type].m_Size,
                                k_VertexTypeInfo[attrib.type].m_Type,
                                stride,
    ...
    if (!useVertexShader) {
        // TODO SetColorMaterial(this->m_GLStates.color0.enable);
    }
}
